...
class ResolutionTest extends FreeSpec with Matchers {
  ...
  def success(elems: (MutablePtr, Folder)*): ValidationNel[ResolutionError, Map[MutablePtr, Folder]] =
  ...
  "Resolution" - {
    ...
    "Follow inside a follow subtree" in {
      val (rootPtr, root: Folder) = (mutablePtr("root"), folder()(
        "friend1" -> follow("friend1", Nil)
      )())

      val (friend1Ptr, friend1Root: Folder) = (mutablePtr("friend1"), folder()(
        "friend2" -> follow("friend2", Nil)
      )())

      val (friend2Ptr, friend2Root: Folder) = (mutablePtr("friend2"), folder(
        "x" -> leaf("x")
      )()())
